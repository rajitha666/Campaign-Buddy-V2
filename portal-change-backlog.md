...
| 11 | Map rendered on top of the sticky topbar once scrolled far enough to overlap it | ✅ | `.map-shell` gets `isolation:isolate` in [`app.css`](campaign-buddy-portal/src/styles/app.css) — boxes Leaflet's internal z-index values into the map's own stacking context so they can never leak above the topbar (or anything else) again, instead of chasing every other element's z-index upward as new cases turn up. |
| 12 | Leave Requests approve/decline row actions rendered as blank pill buttons | ✅ | `ICONS.approve`/`ICONS.decline` were missing entirely (only the tooltip title existed) — added checkmark/cross SVGs in [`Icons.jsx`](campaign-buddy-portal/src/components/Icons.jsx), colored green/red in [`app.css`](campaign-buddy-portal/src/styles/app.css) |

## Batch 5 — Staff Profiles redesign

| # | Change | Status | Files |
|---|--------|--------|-------|
| 13 | Staff picker: unsearchable `<select>` showing only a first name → searchable combobox showing full name + Promoter/Supervisor type; field relabeled "Staff Member" | ✅ | [`StaffProfiles.jsx`](campaign-buddy-portal/src/pages/StaffProfiles.jsx) (reuses the existing `SearchableSelect`), `pageSize:1000` on the staff fetch so the list doesn't silently cap at 25 |
| 14 | Profile card now shows the full HR field set from staff creation (contact, city, reports-to, DOB, gender, addresses, emergency contact, bank details) | ✅ | `GET /staff/:staffId/evaluation` now returns a `profile` object (staff row + resolved `cityName`/`reportsToName`) — [`staff.routes.ts`](campaign-buddy-backend/src/modules/admin/staff.routes.ts) |
| 15 | New "Current activation(s) & performance" card, "Most sold products" card, "Sales & customers approached — by activation" trend chart, and a "Previous activations" table | ✅ | Same endpoint also returns `activations` (full history, per-activation `sales`/`customersApproached`/`isCurrent`), `currentActivationId`, `topProducts` (top 5 by quantity), `customersApproached` (summed `DailyStats.approached`) — all derived from data the endpoint already queried, no schema change |

### Decisions taken
- **"Current activation" shows every activation active today, not just one** — a staff member can rarely be active on two campaigns' activations at once (confirmed in the demo data itself: Sanduni Kumari is concurrently on both "Radiance Q3 Push" and "Sktest Activation"). Falls back to the most recently *ended* activation, labeled "Last activation", only when none is active today.
- **"Customers approached" sums `DailyStats.approached`** (the footfall→approached→converted funnel), not `SalesRecord.otherInterestedCustomers` (a narrower "asked but didn't buy" signal used elsewhere on the Dashboard) — confirmed with the user which metric was meant.
- **The trend chart is "trend over time," not "progress toward a target"** — there's no existing per-staff sales target/quota concept in the system to show progress against, so it's a grouped bar chart (sales + customers approached) per activation, oldest to newest.
- **Sensitive HR fields (bank account number, NIC) are shown in full**, unmasked — this page is already Super Admin / Campaign Admin only, consistent with how the Staff List/Edit form already displays them.
- No new chart library — the trend chart is a small hand-rolled two-series bar chart reusing the existing `.trend-legend`/`.chart-scroll` CSS classes, consistent with the "no library" calls made for `TrendChart`/`CampaignMap` in earlier batches.
