...
import { idealShiftEnd } from "../utils/attendanceWindow";
...
// once a day, setting `checkOutAt` to the activation's explicit `shiftEnd`,
// or the ideal 17:00 local end-of-shift (issue #31) when none is set — the
// same window the rest of attendance uses. A shift whose configured end-time
// would produce a future checkOutAt is checked out at the run time instead.
...
export async function closeOpenShifts(now: Date = new Date(), client: PrismaClient | Prisma.TransactionClient = prisma): Promise<number> {
  const open = await client.attendanceRecord.findMany({
    ...
    include: { activation: { select: { shiftEnd: true } } },
  });

  let closed = 0;
  for (const rec of open) {
    const end = rec.activation.shiftEnd ?? idealShiftEnd(rec.date);
